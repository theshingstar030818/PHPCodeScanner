...
def read_info_from_conf_files():
    ...
    if len(str_tmp) > 0:
        ...
        regex_dynamic_sources = '\\((.*?)(' + str_tmp + '|\\$(?!this|e-)[a-zA-Z0-9_,]*)(.*?)\\)'
    # later more
    # regex_dynamic_sources = '\\((.*?)(\\$_GET\\[.*?\\]|\\$_FILES\\[.*?\\]|\\$_POST\\[.*?\\]|\\$_REQUEST\\[.*?\\]|\\$_COOKIES\\[.*?\\]|\\$_SESSION\\[.*?\\]|\\$(?!this|e-)[a-zA-Z0-9_,]*)(.*?)\\)'
    # regex_dynamic_sources = '\\((.*?)(\\$_GET\\[.*?\\]|\\$_FILES\\[.*?\\]|\\$_POST\\[.*?\\]|\\$_COOKIES\\[.*?\\]|\\$_REQUEST\\[.*?\\]|\\$_FILES\\[.*?\\]|\\$_SERVER\\[.*?\\]|\\$_SESSION\\[.*?\\]|\\$(?!this|e-)[a-zA-Z0-9_,]*)(.*?)\\)'
    # regex_dynamic_sources = '\\((.*?)(\\$_GET\\[.*?\\]|\\$_FILES\\[.*?\\]|\\$_POST\\[.*?\\]|\\$_COOKIES\\[.*?\\]|\\$_REQUEST\\[.*?\\]|\\$_FILES\\[.*?\\]|\\$_SERVER\\[.*?\\]|\\$_SESSION\\[.*?\\]|\\$(?!this|e-)[a-zA-Z0-9_,]*)(.*?)\\)'

    # Now read patterns from badfunctions.json
    with open('conf/badfunctions.json') as f:
        data = json.load(f)

    global patterns
    for item in data['result']:
        pattern = [item['function_name'], item['protected_list'], item['vuln_type']]
...
def check_declaration_of_this_var(file_content, vuln, path, pattern_secure_functions = []):
    ...
    if len(declaration2) > 0:
        return check_declaration_of_this_var(file_content, "$" + declaration2[0][0], path, pattern_secure_functions)

    # $var = $_GET['var']
    regex_decl = re.compile("\\$" + vulnerability + "([\t ]*)=(?!=)(.*)")  # not ==, only =
    declaration = regex_decl.findall(file_content)
    if len(declaration) > 0:
        # check all declarations, later fix this to use the sub content which is located before badfunction
        #
        # for ex:
        # ...
        # echo($var);
        #
        is_secure_protected = False
        for item in declaration:
            # Check if constant
            # decl_text = "$" + vulnerability + declaration[0][0] + "=" + declaration[0][1]
            decl_text = "$" + vulnerability + item[0] + "=" + item[1]
            ...
            rtn = check_is_secure_protected(pattern_secure_functions, decl_text)
            if rtn:
            # detected secure protected line
                is_secure_protected = True

        if is_secure_protected:
            return True, "", ""
                    ...
                    for vulnerable_var in regex.findall(sentence):
                        ...
                        if not check_is_contain_dynamic_sources(vulnerable_var[1]):
                            is_vulnerable, decl_text, line = check_declaration_of_this_var(
                                ...
                                vulnerable_var[1],
                                path,
                                pattern[1]      # added to check secure protected in this function
                            )
